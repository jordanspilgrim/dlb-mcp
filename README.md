...
**Let your AI coding agents leave each other notes.** DLB is a tiny MCP server that gives independent agent sessions (different terminals, even different tools) one shared inbox: hand off a task, drop a heads-up, and actually know when the other agent picked it up. No daemon, no orchestration framework, no ceremony. 10 tools and an optional wake source.
